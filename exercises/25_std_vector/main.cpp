...
int main(int argc, char **argv) {
    {
        ...
        //std::vector 的大小可以在运行时改变，并且当需要更多空间时，它会自动重新分配内存来容纳新增的元素。
    ...
    }
    {
        ...
        {
            // `shrink_to_fit` 是一个非绑定请求，尝试减少容量以匹配大小。
            // 容量可能会减少，但并不保证。
            vec.shrink_to_fit();
            //size() 方法获取当前元素数量，capacity() 获取已分配但可能未使用的空间大小，以及 reserve() 提前分配一定量的空间以提高性能。
    ...
    }
    {
        ...
        //sizeof(vec)：计算 vec 这个 std::vector<int> 对象自身的大小（不是它所管理的数据的大小）。
        //这个值取决于 std::vector 的内部实现，通常包括指向动态分配内存的指针、容量和大小等成员变量。 
        //sizeof(std::vector<int>)：计算一个未初始化的 std::vector<int> 对象的大小。
        //因为这是在编译时确定的，所以它代表了 std::vector<int> 类型对象本身的大小，而不考虑运行时向量中元素的数量或内容。
